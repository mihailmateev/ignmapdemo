...
(function ($) {
	...
	/* pluginName="igTemplating" */
		...
		/* type="object" Used to compile template directives. */
		directives: {
			'if': {
				start: 'if (',
				close: ') {',
		...
		_tokenizeDirectives: function (template) {
			...
			if (template.indexOf(tokens[0]) > 0) {
		...
		_handleCompleteBlock: function (tokens) {
			...
			tmpl = tmpl.replace('{{' + tokens[1], this.directives[tokens[1]].start);
			if (tokens[1] === 'each') {
		...
		_handleEach: function (template, tokens) {
			...
			tmpl = tmpl.replace(/\}\}/, this.directives[tokens[1]].close);
			tmpl = tmpl.replace(tokens[3], this.directives[tokens[1]].end);
